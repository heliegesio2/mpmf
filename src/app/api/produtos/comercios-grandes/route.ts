 ...
 *
 * Lê preços de um concorrente. Não grava nada — só extrai a lista. A comparação
 * e o registro ficam no /analisar.
 *
 * - "fotos" (repetível) + "fonte"="video" -> quadros do vídeo (etiqueta de prateleira)
 *   -> { itens: [{ nome, preco, quadro }] }
 * - "fotos" (repetível) + "fonte"="encarte" -> páginas do encarte (PDF já rasterizado no
 *   navegador) ou fotos -> { itens: [{ nome, preco, imagem, caixa }] }
 */
    ...
    if (fonte === "video") {
      ...
      return NextResponse.json({
        itens: detectados
          ...
          .map((d) => ({ nome: d.nome, preco: d.preco, quadro: d.quadro })),
      });
    }
